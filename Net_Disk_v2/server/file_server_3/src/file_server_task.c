...
/*
实际开始传输文件，需要进行该文件的大小判断来选择是进行大文件模式的传输还是小文件模式的传输
先从传入的cmd和clientfd中获得要传输的文件和路径
再接收从客户端发来的要传输文件的起始地址和实际要传输的大小
开始进行传输
大文件传输模式：进行分片传输
小文件传输模式：直接传输文件
*/
void do_gets_task(int client_fd, char *cmd)
{
    ...
    recvn(client_fd, &offset, sizeof(offset));          //服务端接收文件传输的起始位置
    recvn(client_fd, &part_size, sizeof(part_size));    //服务端接收实际要传输的大小
    printf("offset = %d, part_size = %d\n", offset, part_size);

    bool is_big_file = part_size < big_file_boundary ? false : true;        //实际要传输的大小大于100MB，则为大文件传输模式

    // 发送文件
    if(is_big_file) //大文件传输模式
    {
        int ret = send_big_file(client_fd, fd, offset, part_size);
    }
    else
    {
        int cur_size = 0;
